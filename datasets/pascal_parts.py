...
class PPDataset(Dataset):
    def __init__(self, opts):
        ...
        annotation_folder = f'{opts.dataset_root}/Annotations_Part/'
        images_folder = f'{opts.dataset_root}/VOC2010/JPEGImages/'
        cls = opts.pascal_class
        mat_filenames = os.listdir(annotation_folder)
        voc_list = {str(s) for s in np.loadtxt(f'{opts.dataset_root}/VOC2010/ImageSets/Main/{cls}_{"train" if split == "train" else "val"}.txt', dtype=str)[:, 0]}
