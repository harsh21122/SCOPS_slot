...
class CUB200(torch.utils.data.Dataset):
    ...
    def __init__(self, root, train=True, transform=None, resize=448, req_label=None, three_classes=False, mask_type='gt', crop_to_bbox=False):
        ...
        if self._train:
            ...
            self._test_data, self._test_labels, self._test_parts, self._test_boxes, self._filenames = self._get_file_list(train=False)
    ...
    def __getitem__(self, index):
        ...
        if self._transform is not None:
            if isinstance(self._transform, list):
                ...
                return image, mask, target, parts, boxes, filename
            else:
                image = self._transform(image)

        return image, target, parts, boxes, filename
    ...
    def _get_file_list(self, train=True):
        ...
        if train == True:
            ...
            return test_data, test_labels, test_parts, test_boxes, filenames
