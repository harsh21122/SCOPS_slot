    ...
    for i_iter in range(args.num_steps):
        ...
        if i_iter % args.vis_interval == 0 or i_iter == 100:
            ...
            try:
                ...
                testloader_iter = enumerate(trainloader)
                _, test_batch = trainloader_iter.__next__()
        ...
        if i_iter >= args.num_steps - 1:
            print('save model ...')
            trainer.save_model(osp.join('/home/harsh21122/tmp/models/unsup_parts', 'model_' + str(args.num_steps) + '.pth'), i_iter)
            break
        if i_iter % args.save_pred_every == 0 and i_iter != 0:
            print('taking snapshot ...')
            trainer.save_model(osp.join('/home/harsh21122/tmp/models/unsup_parts', 'model_' + str(i_iter) + '.pth'), i_iter)
